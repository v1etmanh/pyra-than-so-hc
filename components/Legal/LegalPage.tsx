  ...
  return (
    <main className="legal-page">
      <div className="legal-page-inner">
        ...
        <p className="batch-kicker">NUMELYRA / {privacy ? 'PRIVACY' : 'TERMS'}</p>
        ...
        {privacy ? (
          <>
            <section><h2>{vi ? '1. Dữ liệu chúng tôi nhận' : '1. Data we collect'}</h2><p>{vi ? 'NUMELYRA có thể nhận họ tên, ngày sinh, câu hỏi, hồ sơ thần số học, email tài khoản và phản hồi bạn chủ động cung cấp.' : 'NUMELYRA may receive your name, date of birth, questions, numerology profiles, account email, and feedback you choose to provide.'}</p></section>
            ...
            <section><h2>{vi ? '3. Lưu trữ và bên thứ ba' : '3. Storage and third parties'}</h2><p>{vi ? 'Hồ sơ khách được lưu trên thiết bị. Khi đăng nhập, hồ sơ có thể được đồng bộ với Supabase. Nếu bạn đồng ý, NUMELYRA dùng Google Analytics với IP ẩn danh và không gửi ngày sinh nguyên bản vào sự kiện analytics.' : 'Guest profiles are stored on your device. When signed in, profiles may sync with Supabase. If you consent, NUMELYRA uses Google Analytics with IP anonymization and does not send raw birth dates in analytics events.'}</p></section>
            ...
            <section><h2>{vi ? '5. Liên hệ' : '5. Contact'}</h2><p>{vi ? 'Liên hệ chủ sở hữu NUMELYRA qua địa chỉ email hỗ trợ được công bố trên website để yêu cầu truy cập, chỉnh sửa hoặc xóa dữ liệu.' : 'Contact the NUMELYRA owner through the support email published on the website to request access, correction, or deletion of your data.'}</p></section>
          </>
        ) : (
          <>
            <section><h2>{vi ? '1. Bản chất dịch vụ' : '1. Service nature'}</h2><p>{vi ? 'NUMELYRA là công cụ tự phản chiếu và giải trí dựa trên thần số học. Nội dung không phải chẩn đoán tâm lý, y tế, pháp lý, tài chính hay lời tiên đoán chắc chắn.' : 'NUMELYRA is a self-reflection and entertainment tool based on numerology. Its content is not psychological, medical, legal, financial advice, or a guaranteed prediction.'}</p></section>
            ...
            <section><h2>{vi ? '5. Thay đổi dịch vụ' : '5. Service changes'}</h2><p>{vi ? 'NUMELYRA có thể cập nhật, tạm dừng hoặc thay đổi tính năng để bảo đảm an toàn, chi phí và chất lượng vận hành.' : 'NUMELYRA may update, pause, or change features to maintain safety, cost control, and service quality.'}</p></section>
          </>
        )}
