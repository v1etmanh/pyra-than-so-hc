...
const parseDefaultValue = (defaultValue: string, allowEmpty = false) => {
   if (defaultValue) {
   ...
   return allowEmpty ? { date: "", month: "", year: "" } : { date: "01", month: "01", year: "1982" };
};

export const InputDate: FC<PropTypes> = (props) => {
   const t = useTranslations("InputDate");
   const { getValue, defaultValue = "", allowEmpty = false, ...rest } = props;
   const refDate = useRef<HTMLInputElement>(null);
   const refMonth = useRef<HTMLInputElement>(null);
   const refYear = useRef<HTMLInputElement>(null);
   const toast = useToast();
   const initialValues = parseDefaultValue(defaultValue, allowEmpty);
   ...
   useEffect(() => {
      if (defaultValue && defaultValue !== prevDefaultValueRef.current) {
         ...
         const parsed = parseDefaultValue(defaultValue, allowEmpty);
         setDate(parsed.date);
   ...
   return (
      <InputGroup>
         ...
         const newDate = value || (allowEmpty ? "" : "01");
               setDate(newDate);
         ...
         <Input
            onChange={(e: ChangeEvent<HTMLInputElement>) => {
               const value = e.target.value;
               const newMonth = value || (allowEmpty ? "" : "01");
               setMonth(newMonth);
         ...
         <Input
            onChange={(e: ChangeEvent<HTMLInputElement>) => {
               const value = e.target.value;
               const newYear = value || (allowEmpty ? "" : "1982");
               setYear(newYear);
