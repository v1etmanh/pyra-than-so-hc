...
export const RenderItem: FC<PropTypes> = ({ item, onClick }) => {
  ...
  const displayValue = typeof item.value === 'number' && !Number.isFinite(item.value)
    ? 'Đang phân tích'
    : item.value;

  let title = item.name_en && locale === 'en' ? item.name_en : item.name;
  try {
    if (t.has(item.key as any)) {
      title = t(item.key as any);
    }
  } catch {
    title = item.name_en && locale === 'en' ? item.name_en : item.name || item.key;
  }
